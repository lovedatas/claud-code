...
export function buildInteractiveInputFailureMessage(state: {
  stdinIsTTY: boolean
  hasTtyOverride: boolean
  term?: string
}): string | null {
  if (state.stdinIsTTY || state.hasTtyOverride) {
    return null
  }

  const details = ['stdin 不是 TTY，且 /dev/tty 也不可用']
  if (state.term === 'dumb') {
    details.push('TERM=dumb')
  }

  return `当前终端没有可用的交互输入源：${details.join('；')}。这个环境里无法接收键盘输入。`
}

export function getInteractiveInputFailureMessage(): string | null {
  const stdinOverride = getStdinOverride()
  return buildInteractiveInputFailureMessage({
    stdinIsTTY: Boolean(process.stdin.isTTY),
    hasTtyOverride: Boolean(stdinOverride),
